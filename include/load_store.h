...
static inline PsammiteStatusCodes psammite_lpcr(PsammiteVM *vm, uint32_t instruction) {
    ...
    uint64_t value;
    PsammiteStatusCodes code = psammite_read_memory64(vm, vm->_pc+signed_offset, &value);
    if (code != VM_OK) {
        return code;
    }
...
static inline PsammiteStatusCodes psammite_l64(PsammiteVM *vm, uint32_t instruction) {
    ...
    uint64_t value;
    PsammiteStatusCodes code = psammite_read_memory64(vm, final_address, &value);
    if (code != VM_OK) {
        return code;
    }
    psammite_write_register(vm, rd,value);
...
static inline PsammiteStatusCodes psammite_l32(PsammiteVM *vm, uint32_t instruction) {
    ...
    uint64_t final_address = base_address + signed_offset;
    uint32_t value;
    PsammiteStatusCodes code = psammite_read_memory32(vm, final_address, &value);
    if (code != VM_OK) {
        return code;
    }
    psammite_write_register(vm, rd,value);
...
static inline PsammiteStatusCodes psammite_l16(PsammiteVM *vm, uint32_t instruction) {
    ...
    uint64_t final_address = base_address + signed_offset;
    uint16_t value;
    PsammiteStatusCodes code = psammite_read_memory16(vm, final_address, &value);
    if (code != VM_OK) {
        return code;
    }
    psammite_write_register(vm, rd,value);
...
static inline PsammiteStatusCodes psammite_l8(PsammiteVM *vm, uint32_t instruction) {
    ...
    uint64_t final_address = base_address + signed_offset;
    uint8_t value;
    PsammiteStatusCodes code = psammite_read_memory8(vm, final_address, &value);
    if (code != VM_OK) {
        return code;
    }
    psammite_write_register(vm, rd,value);
...
static inline PsammiteStatusCodes psammite_s64(PsammiteVM *vm, uint32_t instruction) {
    ...
    PsammiteStatusCodes code = psammite_write_memory64(vm, final_address, value);
    if (code != VM_OK) {
        return code;
    }
    return VM_OK;
...
static inline PsammiteStatusCodes psammite_s32(PsammiteVM *vm, uint32_t instruction) {
    ...
    PsammiteStatusCodes code = psammite_write_memory32(vm, final_address, value);
    if (code != VM_OK) {
        return code;
    }
    return VM_OK;
...
static inline PsammiteStatusCodes psammite_s16(PsammiteVM *vm, uint32_t instruction) {
    ...
    PsammiteStatusCodes code = psammite_write_memory16(vm, final_address, value);
    if (code != VM_OK) {
        return code;
    }
    return VM_OK;
...
static inline PsammiteStatusCodes psammite_s8(PsammiteVM *vm, uint32_t instruction) {
    ...
    PsammiteStatusCodes code = psammite_write_memory8(vm, final_address, value);
    if (code != VM_OK) {
        return code;
    }
    return VM_OK;
...
static inline PsammiteStatusCodes psammite_lf64(PsammiteVM *vm, uint32_t instruction) {
    uint8_t rs = psammite_decode_itype_rs(instruction);
    uint8_t rd = psammite_decode_itype_rd(instruction);
    uint16_t offset = psammite_decode_itype_immediate(instruction);
    int16_t signed_offset = (int16_t) offset;
    ...
    uint64_t final_address = base_address + signed_offset;
    PsammiteFloat value;
    PsammiteStatusCodes code = psammite_read_memory64(vm, final_address, &value.bits);
    if (code != VM_OK) {
        return code;
    }
...
static inline PsammiteStatusCodes psammite_sf64(PsammiteVM *vm, uint32_t instruction) {
    uint8_t value_reg = psammite_decode_itype_rs(instruction);
    uint8_t address_reg = psammite_decode_itype_rd(instruction);
    uint16_t offset = psammite_decode_itype_immediate(instruction);
    int16_t signed_offset = (int16_t) offset;
    uint64_t base_address = psammite_read_register(vm, address_reg);
    uint64_t final_address = base_address + signed_offset;
    PsammiteFloat value = psammite_read_f_register(vm, value_reg);
    PsammiteStatusCodes code = psammite_write_memory64(vm, final_address, value.bits);
    if (code != VM_OK) {
        return code;
    }
    return VM_OK;
